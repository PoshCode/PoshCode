...
test "Read-Module Supports Packages Includes Both Manifest" {
   ...
   assert {
      "Name = Reflection".MustEqual( ("Name = " + $RequiredModules.Name) )
      "http://poshcode.org/Modules/WASP.psd1".MustEqual( $ModuleManifest.ModuleInfoUri )
      "http://PoshCode.org/Modules/Reflection.psd1".MustEqual( $RequiredModules.ModuleInfoUri )
   }
} -Category Packages
